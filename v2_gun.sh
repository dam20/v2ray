...
show_links() {
  if [ -f "/usr/local/bin/v2ray" ]; then
    ...
    colorEcho ${YELLOW} "Because the Trojan-Go sharing link format has not been finalized, if your client cannot parse this link, please fill in the connection information manually"
    printf "%s:443 %s %s\n" "${sni}" "${uuid}" "${path}tj"
    echo ""
  ...
  if [ -f "/usr/local/bin/v2ray" ]; then
    ...
    if [ -f "/root/.acme.sh/${V2_DOMAIN}_ecc/fullchain.cer" ]; then
      ...
      if [ -f "/root/.acme.sh/${V2_DOMAIN}_ecc/fullchain.cer" ]; then
        ...
        colorEcho ${RED} "Certificate issuance failed, please try again"
        exit 1
      fi
    fi
  else
    colorEcho ${YELLOW} "Please install V2Ray first"
  fi
}
...
install_v2ray() {
  while true; do
    read -rp "Resolved to the domain name of this VPS: "V2_DOMAIN
    if checkIP "${V2_DOMAIN}"; then
      colorEcho ${GREEN} "Domain name ${V2_DOMAIN} is resolved correctly, installation will start soon"
      break
    else
      colorEcho ${RED} "The domain name ${V2_DOMAIN} is parsed incorrectly (yes: force to continue, no: re-enter, quit: leave)"
  ...
  if [ -f "/root/.acme.sh/${V2_DOMAIN}_ecc/fullchain.cer" ]; then
    ...
    if [ -f "/root/.acme.sh/${V2_DOMAIN}_ecc/fullchain.cer" ]; then
      ...
      colorEcho ${RED} "Certificate issuance failed, please run to repair the certificate"
    fi
  fi
}
...
edit_cf_node() {
  ...
  printf "3. %s\n" "www.garmin.com"
  printf "4. %s\n" "amp.cloudflare.com"
  read -p "Enter the new CF node address [leave blank to use the existing value ${cf_node_current}]: "cf_node_new
  case "${cf_node_new}" in
  ...
  sleep 1
  printf "%s\n" "CF node has been changed to ${cf_node_new}"
  show_links
  fi
}
