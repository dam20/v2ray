...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        echo "Please select supported TLS version(default:3):"
        echo "Please note that if you use Quantaumlt X / router / Old version Shadowrocket / Below 4.18.1 Version of V2ray core Please select compatibility mode"
...
menu() {
    ...
    echo -e "Please enter a number: " read -rp "" menu_num
    case $menu_num in
    0)
        update_sh
        ;;
